...
export default function HabitDetailPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Habit Header */}
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 p-8 space-y-4 shadow-xl">
        <div className="flex flex-wrap items-center gap-3">
          <span className="rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 px-3 py-1 text-xs font-semibold">
            {habit.category}
          </span>
          {habit.inRescueMode && (
            <span className="rounded-full bg-amber-500/20 text-[#B58A1C] dark:text-amber-300 border border-amber-500/40 px-3 py-1 text-xs font-bold flex items-center gap-1">
              <LifeBuoy className="h-3.5 w-3.5" /> Rescue Mode Active
            </span>
          )}
        </div>

        <h1 className="font-heading text-3xl font-extrabold text-slate-900 dark:text-white">{habit.name}</h1>
        <p className="text-sm text-slate-600 dark:text-slate-300">{habit.description || 'No custom description provided.'}</p>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-200 dark:border-slate-800/80 text-xs">
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[11px]">Daily Target</span>
            <strong className="text-slate-900 dark:text-white text-sm">{habit.targetAmount} {habit.targetUnit}</strong>
          </div>
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[11px]">Frequency</span>
            <strong className="text-slate-900 dark:text-white text-sm capitalize">{habit.frequency}</strong>
          </div>
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[11px]">Consistency Rate</span>
            <strong className="text-[#40755C] dark:text-emerald-400 text-sm">{completionRate}%</strong>
          </div>
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[11px]">Total Misses</span>
            <strong className="text-[#E07A5F] dark:text-amber-400 text-sm">{missedLogs.length} days</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-6 space-y-4 shadow-sm">
          ...
          {Object.keys(reasonTally).length === 0 ? (
            <p className="text-xs text-slate-500 dark:text-slate-400 italic">No missed logs recorded yet.</p>
          ) : (
                ...
                return (
                  <div key={reason} className="space-y-1">
                    <div className="flex justify-between text-slate-700 dark:text-slate-300 font-medium">
                      <span>{reason}</span>
                      <span className="text-[#B58A1C] dark:text-amber-400 font-bold">{count} times ({percent}%)</span>
                    </div>
        ...
        <div className="space-y-4">
          ...
          {habitInsights.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 text-center space-y-2 shadow-sm">
              <Sparkles className="h-5 w-5 text-amber-500 dark:text-amber-400 mx-auto" />
              <p className="text-xs text-slate-500 dark:text-slate-400">AI is gathering data to detect recurring obstacle patterns.</p>
            </div>
          ) : (
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-6 space-y-4 shadow-sm">
        <h3 className="font-heading text-base font-bold text-slate-900 dark:text-white">Recent Activity History</h3>

        {habitLogs.length === 0 ? (
          <p className="text-xs text-slate-500 dark:text-slate-400 italic">No check-ins logged yet.</p>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {habitLogs.slice(0, 10).map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/40">
                    <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white">{log.date}</td>
                    <td className="py-2.5 px-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        log.status === 'completed' ? 'bg-emerald-500/20 text-[#2D6A4F] dark:text-emerald-300 border border-emerald-500/30' :
                        log.status === 'missed' ? 'bg-amber-500/20 text-[#B58A1C] dark:text-amber-300 border border-amber-500/30' :
                        'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                      }`}>
                        {log.status}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400">{log.failureReason || '—'}</td>
                    <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400 italic">{log.notes || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
